...
# prepare lambda for defined functions
FUNC_EVAL = {
  "abs": lambda x: abs(x),
...
class MagicMath(object):
  ...
  def getFactors(self, x):
    """(int) -> (list)
    Get the factors of an integer x
  ...
  def getFactorial(self, x):
    """(int) -> (int)
    Get the factorial af an integer x
  ...
  def solve(self, o, x, y):
    """(str, float, float)
    Performs arithmetic operations to x and y
  ...
  def assessPrecedence(self, operators):
    """(list)
    Get the highest precedence for list of operators
    """
    highest = 0
    for op in operators:
  ...
  def getIndicesMatchingParen(self, input):
    """(str) -> (tuple)
    Get the indices of matching parenthesis from an input string
  ...
  def evaluateTerm(self, term, is_abs = False):
    """(str, bool) -> (float)
    This is a recursive process of evaluating terms
